...
resource "newrelic_nrql_alert_condition" "container_is_restarting" {
    ...
    nrql {
        query             = "from K8sContainerSample select sum(restartCountDelta) where clusterName in ('crossplane') and namespaceName in ('newrelic','kube-system') and namespaceName not in ('') FACET containerName, podName, namespaceName, clusterName"
    }
...
resource "newrelic_nrql_alert_condition" "container_high_cpu_util" {
    ...
    nrql {
        query             = "from K8sContainerSample select average(cpuCoresUtilization) where clusterName in ('crossplane') and namespaceName not in ('') facet containerName, podName, namespaceName, clusterName"
    }
